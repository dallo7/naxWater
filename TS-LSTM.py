...
def build_lstm_model(lookback, units=50, dropout_rate=0.2, layers=2):
    ...
    # First LSTM layer
    model.add(LSTM(units=units, return_sequences=(layers > 1), input_shape=(lookback, 1)))
...
app.layout = html.Div([
    html.Div([
        html.H1("💧 Water Supply Forecasting with LSTM Neural Networks",
    ...
    html.Div([
        ...
        html.Div([
            html.Div([
                ...
                dcc.Slider(
                    ...
                    step=50,
                    value=0,
                    marks={0: '0 smps', 200: '200 smps', 450: '450 smps', 650: '650 smps', 850: '850 smps', 1050: '1050 smps', },
        ...
        html.Div([
            ...
            html.Div([
                html.Label("LSTM Units:", style={'fontWeight': 'bold'}),
                dcc.Slider(
                    id='units-slider',
                    min=25,
                    max=100,
                ...
                html.P("Hidden units in LSTM layers",
                       style={'fontSize': '11px', 'color': '#7f8c8d', 'marginTop': '5px'})
            ], style={'width': '30%', 'display': 'inline-block', 'padding': '20px'}),
    ...
    html.Div([
        html.Button('🧠 Train LSTM & Forecast', id='forecast-button', n_clicks=0,
...
def update_descriptions(scenario):
    ...
    descriptions = {
        'normal': "📊 Baseline scenario - LSTM will learn temporal patterns from historical data.",
        'missing_data': "🕳️ CHALLENGE 1: Dataset with 20% missing data. LSTM trained on interpolated data.",
        'outliers': "⚡ CHALLENGE 2: Extreme consumption spikes/drops (5%). LSTM may learn outlier patterns.",
        'trend_shift': "📉 CHALLENGE 3: Sudden 30% increase. LSTM can adapt to regime changes with training.",
        'high_noise': "🔍 CHALLENGE 4: High volatility. LSTM's memory cells help filter noise patterns."
    }
...
def update_forecast(n_clicks, scenario, noise_level, forecast_days, lookback, units, num_layers):
    ...
    if model_success:
        training_info = html.Div([
            html.H3("🧠 LSTM Training Results",
                    style={'color': '#2c3e50', 'marginBottom': '15px'}),
            html.Div([
                html.Div([
                    html.Strong("Architecture: "),
                    html.Span(f"{num_layers} LSTM layers with {units} units each")
    ...
    fig_forecast.update_layout(
        title=f'Water Consumption Forecast - LSTM ({num_layers} layers, {units} units) - {scenario.replace("_", " ").title()}',
        xaxis_title='Date',
    ...
    if model_success and history:
        ...
        fig_training.update_layout(
            height=600,
            template='plotly_white',
            title_text='LSTM Training Progress',
            showlegend=True
        )
    else:
    ...
    metrics_div = html.Div([
        html.H3("📊 LSTM Performance & Forecast Summary",
    ...
    if not model_success:
        insights = [
            f"❌ LSTM training failed: {error_msg}",
        ...
        insights_dict = {
            'normal': [
                f"✅ LSTM trained successfully on {train_size} sequences with {lookback}-day lookback",
            ...
            'missing_data': [
                f"✅ LSTM handled interpolated missing data (originally 20% gaps)",
                f"✅ Model learned patterns despite data preprocessing",
                f"⚠️ Interpolation may introduce artificial smoothness",
                "🎯 Evidence: Check if predictions align with interpolated regions"
            ],
            'outliers': [
                f"⚠️ LSTM learned from outlier-contaminated data (5% anomalies)",
                f"✅ Validation MAE: {test_mae:,.0f} m³ - Some outlier influence expected",
                "⚠️ May predict extreme values if outliers seen as patterns",
                "🎯 Evidence: Training loss higher than normal due to outliers"
            ],
            'trend_shift': [
                f"✅ LSTM adapted to 30% trend shift through training",
                f"✅ Sequential learning captures regime changes",
                f"✅ Lookback of {lookback} days helps model context",
                "🎯 Evidence: Validation fit shows adaptation to new consumption level"
            ],
            'high_noise': [
                f"✅ LSTM's memory cells filter noise patterns (noise level: {noise_level:,.0f} m³)",
        ...
        if overfit_ratio > 2.0:
            ...
            insights.append("✅ Excellent generalization - model doesn't overfit")

        bg_color = '#d4edda'
        border_color = '#28a745'

    insights_div = html.Div([
        html.H3("🎯 LSTM Performance Evidence",
                style={'color': '#2c3e50', 'marginBottom': '15px'}),
